...
function initTestimonials() {
  const testimonials = [
    ...
    { text: 'I bought a hand-thrown mug as a gift and my friend was amazed. The artisan even included a handwritten note. Beautiful experience.', author: 'Priya Sharma', role: 'Buyer, Mumbai' },
    { text: 'Finally, a marketplace that values craftsmanship over mass production. Every item I\'ve purchased has exceeded my expectations.', author: 'Elena Rossi', role: 'Art Collector, Milan' },
  ];
...
// ===== BECOME A SELLER (opens signup modal) =====
function initSellerBtn() {
  const btn = document.getElementById('btn-become-seller');
  if (btn && window.openAuthModal) {
    btn.addEventListener('click', (e) => { e.preventDefault(); window.openAuthModal('signup'); });
  }
}
